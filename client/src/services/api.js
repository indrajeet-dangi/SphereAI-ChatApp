...
export const auth0LoginUser = async (payload) => {
  const { data } = await api.post("/auth/auth0-login", payload);
  return data;
};
...
export const addGroupMembers = async (groupId, members) => {
  const { data } = await api.put(`/groups/${groupId}/add-members`, { members });
  return data;
};

export const removeGroupMember = async (groupId, memberId) => {
  const { data } = await api.put(`/groups/${groupId}/remove-member/${memberId}`);
  return data;
};

export const makeGroupAdmin = async (groupId, userId) => {
  const { data } = await api.put(`/groups/${groupId}/make-admin`, { userId });
  return data;
};

export const updateGroupDescription = async (groupId, description) => {
  const { data } = await api.put(`/groups/${groupId}/description`, { description });
  return data;
};

export const exitGroup = async (groupId) => {
  const { data } = await api.post(`/groups/${groupId}/exit`);
  return data;
};
...
export const generateGroupSummary = async (groupId) => {
  const { data } = await api.post(`/groups/${groupId}/generate-summary`);
  return data;
};

export const markGroupSeen = async (groupId) => {
  const { data } = await api.post(`/groups/${groupId}/mark-seen`);
  return data;
};
...
export const correctGrammar = async (payload) => {
  const { data } = await api.post("/ai/correct-grammar", payload);
  return data;
};
